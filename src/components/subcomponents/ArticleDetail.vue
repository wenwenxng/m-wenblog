    ...
    <div class="articledetail-container">
        <div class="mui-scroll-wrapper" id="refreshContainer1">
            <div class="mui-scroll">
                <h3>{{ detail.art_title}}</h3>
                <p><span class="mui-icon mui-icon-contact">{{ detail.user_name }}</span><span
                        class="mui-icon-extra mui-icon-extra-outline">{{ detail.art_cre_time | dateFormat }}</span><span
                        class="mui-icon mui-icon-eye">{{ detail.art_view }}</span></p>
                <hr>
                <div class="content">
                    <img src="/static/images/menu/menu1.png" alt="">
                    <p>{{ detail.art_content }}</p>
                </div>
                <comment-box :comments="comments" @getMulti="getMulti" @handleSecondPostClick="handleSecondPostClick"></comment-box>
...
<script>
    ...
    import loginBox from './LoginComponent.vue'
    ...
    export default {
        ...
        methods: {
            getArticleDetail() {
                this.$axios.get('/api/article/queryArticleDetail.php?art_id=' + this.id)
                    .then(res => {
                        if (res.data.error) {
